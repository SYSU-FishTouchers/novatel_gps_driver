...
sensor_msgs::Imu get_imu_msg(const vector<string> &, int, string);

serial_port::BDSephem get_ephem_msg(const vector<string> &, int);

serial_port::Gprmc get_gprmc_msg(const vector<string> &, int);

vector<string> time_correct(vector<string> gprmc_body, int time_zone);
...
int main(int argc, char **argv) {
    ...
    if ((time_zone > 12) || (time_zone < (-12))) {
        cout << endl;
        ROS_ERROR("Time zone setting error, please reset!\n");
        return 1;
    }
    ...
    while (ros::ok()) {
        ...
        if (check_msg(data) == "RANGEA") {
            ...
            if (!check) continue;
            // 发布 range_msg
            sensor_msgs::Imu imu_msg = get_imu_msg(imu_body, count_i, imu_frame_id_param);
            ...
            // 时区时间校正
            if (time_zone) {
                gprmc_body = time_correct(gprmc_body, time_zone);
            }
            // 发布 gprmc_msg
            ...
            ROS_WARN("Unkonwn data, will not be processed: %s", data.c_str());
...
// 将 imu 数据转换成 ros 消息
sensor_msgs::Imu get_imu_msg(const vector<string> &imu_body, int count, string imu_frame) {
